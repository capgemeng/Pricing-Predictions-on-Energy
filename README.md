...
**SARIMAX** provide the best results as the time series part is more present than the external variables part. If the external variables alone could explain a lot and this complemented by a part of autocorrelation or seasonality, supervised models might be the better choice. 

We use SARIMAX with producer energy prices lag-1..lag-12 as exogenous variables (lag-1 one month, lag-12 twelve months before). For this multivariate model, we use the auto_arima function imported from pmdarima.arima. We obtain the following metrics for our test sample: 
...
This solution, while providing best results with the test data, it is rejected. We cannot use it for forecasting as it is necessary to specify the future values of the exogenous variable when forecasting. A variable like producer energy prices cannot work, as you will not know which prices will occur in the future. Whereas, a variable like holidays can work, as you know which holidays will occur in the future.
...
We also obtain good results using **Ensemble with StandardScaler, PCA and XGBRegressor**. We predict the producer energy prices using as predictors producer energy prices from lag-1 to lag-12. Adding these variables make the model multivariate, which was required for this user case. Our hyperparameters for:
* PCA: n_components = 2
...
This provide us the following energy producer prices forecast for next 100 lags (months):
...
We have performed a multistep forecast by iterating over the forecasted values and redoing one-step forecasts every time. For this particular problem, we deem forecasting 100 lags (months) adds much errors. We should be rather more cautious here, by retraining the model every time we got new data, and forecasting just one lag or just only a few lags (months). This can be done by using REST APIs, e.g. by using Flask or AWS SageMaker, with different endpoints to retrain and forecast.
...
Concerning the best prediction models for energy producer prices in Europe,  **SARIMAX** provided the best results with the test sample. This model is rejected for forecasting, as it's necessary to specify the future values of the producer energy prices (exogenous variable) which are unknown . 

The **Ensemble with StandardScaler, PCA and XGBRegressor** adding a potential seasonality with time lagging provided also good results. We can perform a multistep forecast by iterating over the forecasted values and redoing one-step forecasts every time. In this case, it's taking the forecasted value as a past data point at the risk of adding up more and more errors. For this particular problem, we deem forecasting 100 lags (months) adds much errors. Although it has been done as requested,  we should be rather more cautious here, by retraining the model every time we got new data, and forecasting just one lag or just only a few lags (months).

**The prices of energy forecast reveal that prices will decrease in 2023**.
